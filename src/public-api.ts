...
function idempotencyKey(config: ZeishConfig): string {
  ...
  throw new Error('Provide createIdempotencyKey when crypto.randomUUID is unavailable.');
}
...
export async function request<T>(
  ...
  if (!fetchImpl) throw new Error('A Fetch API implementation is required to use the Zeish public API.');
  ...
  if (!response.ok) {
    throw new ZeishApiError(response.status, await response.text(), init.method ?? 'GET', path);
  }
...
export function createZeishApi(config: ZeishConfig): ZeishPublicApi {
  const sandboxPath = (sandboxId: string) => `/public/sandboxes/${encodeURIComponent(sandboxId)}`;
  const lifecycle = (sandboxId: string, action: 'start' | 'pause' | 'resume' | 'stop' | 'kill') =>
  ...
  return {
    ...
    listSandboxes: (options: ZeishPageOptions = {}) =>
      request<ZeishSandboxPage>(config, `/public/sandboxes${queryString(options)}`),
    getSandbox: sandboxId => request<ZeishSandbox>(config, sandboxPath(sandboxId)),
    destroySandbox: sandboxId =>
      request<ZeishSandbox>(config, sandboxPath(sandboxId), mutation(config, { method: 'DELETE' })),
    getExecAccess: sandboxId =>
      request<ZeishAccess>(config, `${sandboxPath(sandboxId)}/exec-access`),
    getTerminalUrl: sandboxId =>
      request<ZeishTerminalUrlResponse>(config, `${sandboxPath(sandboxId)}/terminal-url`),
    ...
    listLogs: (sandboxId, options: ZeishListLogsOptions = {}) =>
      request<ZeishLogEntry[]>(config, `${sandboxPath(sandboxId)}/logs${queryString(options)}`),
    listEvents: (sandboxId, options: ZeishListEventsOptions = {}) =>
      request<ZeishSandboxEvent[]>(config, `${sandboxPath(sandboxId)}/events${queryString(options)}`),
    ...
    createSnapshot: (sandboxId, displayName) =>
      request<ZeishSnapshot>(
        ...
        mutation(config, { method: 'POST', body: JSON.stringify({ displayName }) })
      ),
    listSnapshots: sandboxId => request<ZeishSnapshot[]>(config, `${sandboxPath(sandboxId)}/snapshots`),
